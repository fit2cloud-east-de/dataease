...
<script>
import { isKettleRunning, listPiApiDatasource, post } from '@/api/dataset/dataset'
...
export default {
  ...
  data() {
    return {
      ...
      dataSource: '',
      dsLoading: false,
      pi_api_info: {point_name:null,data_rate:null,start_time:null,end_time:null},
      tables: [],
      checkTableList: [],
      mode: '0',
      syncType: 'sync_now',
  ...
  methods: {
    ...
    loadData(){
      ...
      this.dbPreview({
        ...
        info: JSON.stringify(this.pi_api_info)
      })
    },
    ...
    initDataSource() {
      listPiApiDatasource().then((response) => {
