...
@Service("extDorisDDLProvider")
public class DorisExtDDLProvider extends DefaultExtDDLProvider {

    private static final String creatTableSql =
            "CREATE TABLE `TABLE_NAME`" +
                    "Column_Fields" + " UNIQUE KEY(`id`)\n" +
                    "COMMENT 'OLAP'\n" +
        ...
        if (create) {
            list.add(
                    ExtTableField.TableField.builder()
                            ...
                            .comment("ID")
                            .primaryKey(true)
                            .type(ExtTableField.BaseType.varchar)
                            .size(128)
                            .build()
            );
        }
